...
function htmlToDocxParagraphs(html) {
  ...
  Array.from(wrapper.children).forEach((el) => {
    if (el.tagName === 'H1') addParagraph(el, { heading: docx.HeadingLevel.HEADING_1 });
    else if (el.tagName === 'H2') addParagraph(el, { heading: docx.HeadingLevel.HEADING_2 });
    else if (el.tagName === 'H3') addParagraph(el, { heading: docx.HeadingLevel.HEADING_3 });
    else if (el.tagName === 'UL') {
      Array.from(el.children).forEach((li) => addParagraph(li, { bullet: { level: 0 } }));
    } else if (el.tagName === 'OL') {
      Array.from(el.children).forEach((li) => addParagraph(li, {
